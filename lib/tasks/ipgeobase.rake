...
namespace :ipgeobase do
  ...
  namespace :upload do
    ...
    task :cities => :environment do
      russia  = Ipgeobase::IpgeobaseRegion.where( name: 'Россия' ).first_or_create
      ukraine = Ipgeobase::IpgeobaseRegion.where( name: 'Украина' ).first_or_create
      ...
      content.each_line do |c|
        ...
        3.times do |t|
          region = names_depth_cache.last.children.where( :name => options[3-t] ).first || Ipgeobase::IpgeobaseRegion.new(:name => options[3-t])
    ...
    task :ips => :environment do
      ...
      Ipgeobase::IpgeobaseIp.delete_all
      content.each_line do |c|
        options = c.split("\t")
        next if options.last.to_i.zero?
        Ipgeobase::IpgeobaseIp.create( :start_ip => options.first, :end_ip => options.second, :ipgeobase_region_id => options.last )
      end
